		...
		while (true)
		{
			...
			if (_stricmp(decision.c_str(), "Advance") == 0)
			{
				if (player->canAdvance()) {
					...
					shared_ptr<Order> order(new Advance(numOfArmies, sourceTerritory, targetTerritory, player->getTerritoryList(), playerTerritories[targetTerritory->ownerID - 1], player->getCapturedTerritory(), &playersNegotiated));
					player->getOrderList()->addOrder(order);
				}
			...
			else if (_stricmp(decision.c_str(), "Airlift") == 0)							//case where player chooses airlift
			{
				if (player->getHand()->findCardType("Airlift"))								//check if player has atleast 1 airlift card
				{
					...
					shared_ptr<Order> order(new Airlift(numOfArmies, sourceTerritory, targetTerritory, player->getTerritoryList(), playerTerritories[targetTerritory->ownerID - 1], player->getCapturedTerritory(), &playersNegotiated));
					player->getOrderList()->addOrder(order);
