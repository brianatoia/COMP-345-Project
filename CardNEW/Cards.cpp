...
vector<string>* Card::getPtrCardTypeArr()
{
	return &vecCardType;
}
...
void Deck::printVecDeck()
{
	cout << "\n The current vecDeck contains " << vecDeck.size() << " cards. \n" << endl;
	for (int i = 0; i < vecDeck.size(); i++) {
		cout << "  #" << i << " card is " << vecDeck.at(i)->getCardType() << endl;
	}
}
...
{
	//use push_back to add the card into vecDeck_cards
...
{
	//add the card into the hand,we can recall the deck->draw() to get a card
	vecHand.push_back(aCard);

}

void Hand::printVecHand()
{
	if (vecHand.size() > 0) {
		cout << "this hand has " << vecHand.size() << " cards " << endl;
		for (int j = 0; j < vecHand.size(); j++) {
			cout << "  #" << j << " is " << vecHand.at(j)->getCardType() << endl;
		}
	}
	else {
