...
void Player::issueOrder(string orderType)
{
    if (orderType == "Deploy")
    {
        ...
        for (shared_ptr<Territory> t : territoryList)
        {
            if (transform(name.begin(), name.end(), name.begin(), ::tolower) == transform(t->name.begin(), t->name.end(), t->name.begin(), ::tolower))
            {
                territory = t;
                break;
            }
        ...
        shared_ptr<Order> order(new Deploy(10, territory,territoryList));
        order->execute();
